...
create table if not exists public.store_settings (
  ...
  logo_path text,
  qr_image_path text,
  shipping_fee numeric(10,2) not null default 6,
  free_shipping_threshold numeric(10,2) not null default 75,
  return_policy text not null default 'Returns are accepted within 7 days for items that differ materially from their listed condition.',
...
-- Atomic checkout: validates inventory, calculates totals from database prices,
...
begin
  if p_payment_method not in ('COD','QR') then raise exception 'Unsupported payment method'; end if;
  v_method := p_payment_method::public.payment_method;
  if v_method = 'QR' and (coalesce(trim(p_transaction_id),'') = '' or coalesce(trim(p_payment_proof_path),'') = '') then
  ...
  -- First pass locks every product row and validates stock.
  ...
  select shipping_fee, free_shipping_threshold into v_shipping_fee, v_free_threshold from public.store_settings where id = 1;
  v_shipping := case when v_subtotal >= coalesce(v_free_threshold,75) then 0 else coalesce(v_shipping_fee,6) end;

  if coalesce(trim(p_promo_code),'') <> '' then
    select * into v_promo from public.promo_codes where upper(code)=upper(trim(p_promo_code)) and active=true and expires_at >= now() for update;
    if found then
      if v_promo.discount_type='percentage' then v_discount := round(v_subtotal * (v_promo.value/100.0),2);
      else v_discount := least(v_subtotal,v_promo.value); end if;
    end if;
  end if;

  v_total := greatest(0, v_subtotal + v_shipping - v_discount);
  v_status := case when v_method='QR' then 'Payment Verification Required'::public.order_status else 'Pending'::public.order_status end;

  insert into public.orders(id,public_order_id,customer_id,email,full_name,phone,address,city,postal_code,subtotal,shipping,discount,total,payment_method,transaction_id,payment_proof_path,status,promo_code)
  values(v_order_id,v_public_order_id,p_customer_id,p_email,p_full_name,p_phone,p_address,p_city,p_postal_code,v_subtotal,v_shipping,v_discount,v_total,v_method,p_transaction_id,p_payment_proof_path,v_status,nullif(trim(p_promo_code),''));
  ...
  loop
    ...
    update public.products
      set inventory = case when one_of_one then 0 else greatest(0,inventory-v_qty) end
      where id=v_product.id;
  end loop;

  if found and v_promo.id is not null then update public.promo_codes set usage_count=usage_count+1 where id=v_promo.id; end if;
...
create policy "customers view own orders" on public.orders for select using (customer_id=auth.uid() or public.is_admin(auth.uid()));
...
-- If a QR proof is rejected, release the reserved inventory exactly once.
create or replace function public.restore_inventory_on_qr_rejection()
returns trigger language plpgsql security definer set search_path=public as $$
begin
  if new.payment_method='QR'
     and new.status='Payment Rejected'
     and old.status is distinct from 'Payment Rejected' then
    update public.products p
      set inventory = case when p.one_of_one then 1 else p.inventory + oi.quantity end
...
create trigger orders_restore_inventory_on_reject
before update of status on public.orders
for each row execute function public.restore_inventory_on_qr_rejection();
